...
char *channel_names[] = {"pH", "EC"};   // <-- CHANGE THIS. A list of channel names (this list should have TOTAL_CIRCUITS entries)
...
const unsigned int blink_frequency = 2000;     // the frequency of the led blinking, in milliseconds
...
void do_sensor_readings() {
  ...
  if (request_pending) {                          // is a request pending?
    ...
    while (sSerial.available()) {                 // while there is data available from the circuit
      ...
      // keep track of status messages recieved with each reading. 
      if (!twice && c == '\r') {
        sensordata[sensor_bytes_received] = ' ';
        sensor_bytes_received++;
        twice = true;
      }
      else if (c=='\r') {                              // in case it's a <CR> character, we reached the end of a message
